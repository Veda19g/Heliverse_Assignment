...
import { viewSingleStudent, updateSingleStudent } from '@/actions';
...
export default function UpdateStudent() {
    ...
    useEffect(() => {
        const fetchStudent = async () => {
            const student = await viewSingleStudent(studentId);
            if (student) {
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await updateSingleStudent(studentId, updatedStudent);
        if (result) {
            alert('Student updated successfully!');
            router.push('/teacher/classroom'); // Redirect after successful update
        }
    };
